...
class ExportDialog(QDialog):
    ...
    def _on_run(self) -> None:
        ...
        try:
            ...
            if fmt == "json":
                self._export_json(pairs, include_imgs, relative)
            elif fmt == "yolo":
                self._export_yolo(pairs, include_imgs)   # YOLO는 항상 상대좌표
            elif fmt == "coco":
                self._export_coco(pairs, include_imgs, relative)
            log.info(f"내보내기 완료 — {len(pairs)}개 → {self._out_dir}")
            QMessageBox.information(
                self, t("export.title"),
                t("export.done").format(n=len(pairs)),
            )
            self.accept()
    ...
    def _export_json(self, pairs, include_images: bool, relative: bool) -> None:
        ...
        for i, (img_path, anns) in enumerate(pairs, 1):
            ...
            self._lbl_status.setText(f"{i} / {len(pairs)}  {img_path.name}")
