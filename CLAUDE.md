...
4. `git push`는 **어떤 브랜치에 올리는지가 명확하면 별도 확인 없이 바로 진행**한다(예: 사용자가 "push해줘"라고 했고 지금 작업 중인 브랜치가 하나뿐이면 그 브랜치). 단, 워크트리가 여러 개거나 여러 브랜치가 동시에 걸려 있는 등 **대상 브랜치가 하나로 정해지지 않으면 반드시 먼저 물어보고** 진행한다(2026-08-26 사용자 확정 — "push 자체"가 아니라 "어느 브랜치인지"가 확인 대상). 커밋 자체는 아래 "Git 커밋 규칙"대로 즉시 수행한다.
