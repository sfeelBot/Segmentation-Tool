...
## 2026-08-28 — installer 시작 splash

- `main.py`에서 QApplication과 프로그램 방식 splash를 먼저 띄운 뒤 무거운 라이브러리를 로드.
- 단계별 메시지를 즉시 처리하고 프로젝트 선택/메인 창 준비 완료 시 splash 자동 종료.
- 새 이미지·의존성 없이 기존 PyQt6만 사용.
- 스모크에서 발견된 접근 불가 최근 프로젝트 경로는 `recent()`가 안전하게 건너뛰도록 보완.
...
- 일괄 추론 중 체크포인트/모델 가중치를 한 번만 준비하도록 공통 엔진 경로 개선.
- 최고 IoU 모델과 epoch별 train/val loss·IoU JSON/PNG 저장 구현.
- 커밋: `ca30948`.
...
- 기존 JSON이 손상됐거나 최상위 객체가 아니면 덮어쓰지 않고 예외를 전달해 일괄 처리의 부분 실패로 집계하도록 강화했다.

## 2026-08-27 — 오토라벨링 원본 해상도 슬라이딩 윈도우 추론

- 상태: 구현 완료 / 검증 대기 / 커밋 없음
- `AutoLabelWorker`의 체크포인트·모델 1회 로드는 유지하고, 이미지별 추론만 전체 resize에서 원본 픽셀 좌표 기반 패치 추론으로 변경했다.
- 학습 입력 크기를 패치 크기로 사용하며 64px 겹침 영역의 softmax 확률을 평균 병합한다. 패치는 최대 4개씩 배치 추론한다.
- 패치보다 작은 이미지는 우측·하단을 edge padding하고 결과를 원본 크기로 crop한다. 모델 출력 해상도가 패치와 다르면 logits를 패치 크기로 보간한다.
- `tests/test_auto_label_sliding.py`에 작은 이미지, 마지막 경계 패치, 겹침 병합·배치 추론, 출력 크기 보정 검증을 추가했다.
- 작은 패치에서 stride가 1px로 축소되지 않도록 겹침을 패치 축 길이의 최대 25%(상한
  64px)로 제한하고, 큰 이미지 한 장을 처리하는 중에도 패치 배치 사이에서 취소 요청을
  반영하도록 보완했다.

## 2026-08-27 — 학습 중 프로젝트 데이터 잠금

- 학습 큐가 활성화된 동안 창 닫기, 프로젝트 전환, 어노테이션 가져오기를 경고 팝업으로 차단했다.
- 라벨링의 이미지 추가/폴더 추가/삭제, 클래스 추가/삭제/색상 변경, 캔버스 편집/삭제/실행 취소/OK/오토라벨링/이전 라벨 복사 진입점에 공통 변경 가드를 연결했다.
- 이미지 탐색, 확대/축소, 패닝, 파일명 복사는 계속 허용한다.
- 학습 시작 직전에 캔버스의 지연 저장을 동기 반영하며, 이미지 폴더 가져오기가 진행 중이면 시작을 보류한다.

## 2026-08-27 — 학습 파이프라인 병목 완화 및 계측

- 상태: 구현 완료 / 검증 완료 / 커밋 없음
- 검증 예측과 정답 전체를 epoch 끝까지 보관하던 방식을 고정 크기 confusion matrix 누적으로 변경하고 기존 `compute_metrics()` 결과 호환성을 유지했다.
- validation DataLoader는 별도 worker를 만들지 않도록 `num_workers=0`, `persistent_workers=False`로 고정하고 CUDA에서는 pinned memory를 사용한다.
- 학습 이미지 캐시 총 512 MiB를 worker 수로 나누고 validation 캐시는 64 MiB로 제한해 다중 worker의 캐시 메모리 증폭을 줄였다.
- epoch 메트릭과 로그에 data/train/validation/metric/total 시간을 기록하며 체크포인트 저장 시간도 로그에 남긴다.
- 배치 loss의 GPU 동기화는 한 번만 수행하고, UI 신호는 최대 0.1초 간격과 epoch 마지막 배치에만 보내도록 제한했다.
- loss 차트의 train point는 최대 2,000개로 유지해 장시간 학습 시 재계산 및 redraw 비용 증가를 제한했다.
- `tests/test_training_performance.py`에 streaming metric의 기존 결과 동등성과 고정 메모리 누적 검증을 추가했다.

- 학습 시작 preflight에서 저장 타이머뿐 아니라 이미 시작된 백그라운드 annotation 저장
  스레드도 완료될 때까지 기다려, 잠금 직전의 지연 저장이 학습 데이터에 뒤늦게 섞이는
  경합을 차단했다.

## 2026-08-27 — 학습 설정 마우스 휠 변경 차단

- 상태: 구현 완료 / 검증 대기 / 커밋 없음
- `ConfigForm`의 모든 숫자 입력과 콤보박스에서 wheel event를 차단해 설정 폼을
  스크롤하다 학습 인자가 우발적으로 바뀌지 않도록 했다.
- 직접 입력, 화살표 버튼 및 프로그램 방식의 값 변경은 그대로 유지했다.

## 2026-08-27 — 학습 큐 중지 응답 개선

- 상태: 구현 완료 / 검증 대기 / 커밋 없음
- 중지 요청 시 현재 GPU batch까지만 안전하게 완료하고 validation·metric·checkpoint로
  진행하지 않은 채 현재 작업을 종료하도록 변경했다.
- validation 도중에도 batch 경계마다 중지 플래그를 확인하며, 중복 중지 요청을 막기 위해
...
## 2026-08-28 — GitHub #27 Python 3.12 빌드 탐색 zone 반영
...
- BUG-027: 성공 저장 뒤 `_save_failed_once`를 리셋하지 않아 저장 실패 경고가 앱
  세션 동안 한 번만 표시된다.
...
## 2026-08-31 — Zone 에디션 사용자 매뉴얼 초안 (실 UI 캡처 기반 HTML)

### 산출물
- `docs/USER_MANUAL_ZONE.html` — 현장 사용자용 한국어 매뉴얼 초안(목차 앵커, 밝은
  테마, 상대경로 `<img>` 참조, base64 인라인 없음).
- `docs/manual_assets_zone/*.png` — **실제 화면 캡처 32장**. 헤드리스/목업이 아니라
  진짜 데스크톱에 앱을 띄우고 `PIL.ImageGrab.grab(bbox=frameGeometry)`로 촬영했다.
  콜아웃 번호는 눈대중이 아니라 `widget.mapToGlobal()`로 얻은 실제 스크린 좌표에
  `PIL.ImageDraw`로 합성했고, 위젯 텍스트를 가리지 않도록 마커를 비켜 찍고
  지시선으로 대상을 가리키는 방식으로 그렸다.

### 다룬 범위 (실사용 순서)
프로젝트 시작 다이얼로그 → 메인 5탭 구조 → 모델 탭(프리셋 팝업/검증/로드) →
라벨링 탭(패널 구성 + 도구 7종 툴바) → 학습 탭(설정·큐·진행창·손실 그래프·
체크포인트) → 추론 탭(체크포인트 선택·실행·결과/범례) → **존 분석 탭 21장**:
이미지·체크포인트 직접 열기, 추론, 타겟 클래스 지정, AI신뢰도/픽셀 threshold,
민감도+원 자동 검출, 원 선택/이동/반지름/우클릭 지름 변경·삭제, 존 목록과
하이라이트, 편집 도구 5모드 배타 툴바 + 브러시 그리기/지우기 + 블랍 삭제 + Undo,
이미지별 사이드카 자동 저장, Zone 결정 방법 3모드, 일괄 처리(기존 존 충돌 팝업),
결과 Long/Wide 뷰, Excel 내보내기.

### 캡처 하네스 (스크래치패드, 저장소 미포함)
`capture_manual.py` — 앱을 실제로 구동한 뒤 실 위젯 API(버튼 click, QAction 토글,
합성 QMouseEvent 드래그)로 조작하며 단계마다 캡처. `QFileDialog` 정적 메서드만
하네스에서 대체해 네이티브 파일창 없이 실제 코드 경로를 그대로 실행했다. 앱 코드는
한 줄도 수정하지 않았다. 데모 데이터는 `projects/nok`(실사용 데이터)를 읽기 전용으로
복사·축소한 `projects/manual_demo`(gitignore 대상)이며, 캡처용 체크포인트는 학습
탭에서 실제로 20 epoch 학습해 만들었다.

### 미해결 / 다음 라운드
- `30_zone_batch_progress`(일괄 처리 진행창)만 캡처하지 못했다 — 캐시된 추론 결과로
  배치가 순식간에 끝나 진행 창이 표시 임계시간을 넘기지 못한다. 해당 자리는 텍스트
  설명으로 대체했다.
- 일괄 처리 결과 다이얼로그는 앱이 `exec()`(모달 중첩 루프)로 띄우기 때문에 하네스의
  `processEvents` 루프가 그 안에서 빠져나오지 못한다. 앱 수정 없이, 워커 시작 직후
  하네스가 `completed` 시그널을 넘겨받아 같은 다이얼로그(실제 위젯·실제 15행 데이터)를
  비모달로 띄워 캡처했다. 앱 사용자 동작에는 영향 없는 자동화 한계.
- 이 문서는 **초안**이다. 검토 피드백 반영 최종본에서 README 링크 추가·push 예정.
