...
> **[최신, 2026-08-20]** "아이콘 목업 승인 + 학습/추론 UI 재편 재개 + 전반적 디자인 톤
> 정리" 요청 → 디자인 에이전트 홀리스틱 검토 → 통합 Artifact(`7876ed3e`) + 발견 8건 →
> 사용자 승인 → **7단계 실행안**([roadmap.md](../roadmap.md) "디자인 톤 홀리스틱 재검토"
> 절: ①아이콘SVG ②장식이모지제거 ③model_tab팔레트 ④loss_chart배색 ⑤학습/추론서브스플리터
> ⑥i18n en전환 ⑦팔레트정규화). **①~⑤ 전부 구현+독립검증 통과**:
> ①커밋`2ad0165`+`ed99ca5` / ②(1차 에이전트 세션한도 중단→2차가 이어받아 완료)
> 커밋`b33491d`+`de9722a` / ③커밋`a8ac52d`+`386b69b` / ④커밋`e76361a`+`9c7916c`
> / ⑤학습`a32a8b5`+`b51318f`·추론`b09fb83`+`2955da7`(신규 `inference_image_list.py`),
> **조건부 통과**(골든패스 전부 확인, 크래시·데이터손실 없음, 발견 3건은 아래).
> **⑥~⑦ 미착수**, BUG-008 재검증 완료 후 ⑥(i18n en전환) 진행 예정.
> 부수 발견 버그: **BUG-005**(P2, 라벨링 탭 폴더그룹핑 죽은 코드, GitHub #1 VOC 정정
> 필요 — `decisions-needed.md` 등록, **사용자 결정 대기**, 7단계와 독립) /
> **BUG-006**(P2, model_tab 검증기/로더 정책 불일치, 이전부터 존재) / **BUG-007**(P3,
> loss_chart 색 중복 — 리더가 직접 수정, Closed) / **BUG-008**(P2, 서브스플리터 5곳
> 핸들 끝까지 드래그 시 0px 붕괴 — 리더가 직접 `setChildrenCollapsible(False)` 추가해
> 수정(커밋 `7a98760`), **재검증 진행 중**) / **BUG-009**(P2, 추론 탭 검색 해제 시
> 네비 카운터 표시 오류, 데이터손상 없음, Open) / **BUG-010**(P3, 폴더트리 펼침화살표
> 미표시, 더블클릭은 정상, Open).
>
> **GitHub 이슈 라운드2(#3~#7) 기획 완료** — planner가 스펙 작성:
> [voc-github-issues-round2-2026-08-20.md](../specs/voc-github-issues-round2-2026-08-20.md).
> 코드 기준 직접 검증 완료 — **#6-B(어노테이션 개수비례 로딩지연, 사용자 "반드시 필요"
> 명시)** 원인 특정: `labeling_tab.py` `_refresh_ann_list()` + `annotation_canvas.py`
> `_rebuild_overlay()` 둘 다 매 편집·전환마다 O(n) 전체 재구축 — 난이도 중~상, 별도
> 라운드+대규모 벤치마크 권장. **#6-A(깜빡임)** `_invalidate_overlay()` 타이밍버그,
> 원인 국소화(함수 1개), 난이도 하~중. **#7** 사용자 주장 사실 확인(OK 처리 시 라벨
> 존재해도 팝업 없음) — 기존 "전체삭제" 확인 패턴 재사용 가능, 난이도 하. **#4** 클립보드
> 복사 패턴 재사용 가능(난이도 하), 오토라벨 patch-training 여부 질문엔 "학습 기본값은
> patch, 단 오토라벨링 추론 자체는 전체이미지 리사이즈라 무관"으로 답변 완료(기존
> 갭으로 문서화만, 수정 요청 아님). **#3** 우클릭이 이미 전역 패닝에 배정돼 있어
> 사용자 요청과 충돌 확인, 더블클릭은 미사용이라 안전, 잘림 버그는 원인 확정(고정폭
> QSpinBox+패딩) 저위험. **결정 대기 등록**: #5(모델탭 구조변경, 3탭이 `loaded_model`
> 교차참조 확인됨 — go/no-go 필요) / #3(더블클릭 시 정확한 동작 방식 + 우클릭 대안 여부).
> **나머지(#3부속·#4·#6-A·#6-B·#7)는 결정 없이 바로 구현 착수 가능.**
>
