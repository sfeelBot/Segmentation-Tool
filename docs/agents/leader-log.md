...
>
> **라운드 2(라벨링 탭) 구현 완료, 검증 대기**: 좌/우 패널 세로 QSplitter 전환(초기 비율
> 3:2/2:3 재현) + `ClassPanel` 200px 상한 제거, 전체화면 토글 회귀 없음 확인. 커밋 `f086636`
> (feat)+`9901681`(docs), push 안 함. 독립 검증(실제 목업 대비 육안 확인 포함) 위임 예정.
>
