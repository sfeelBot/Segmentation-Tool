...
> 미표시, 더블클릭은 정상, Open).
>
> **[병행] GitHub 신규 이슈 4건(#3~#6, 2026-08-20) 기획 착수** — 리더가 WebFetch로 원문
> 확인(브러시 크기조절 인터랙션+입력영역 잘림 #3, 이미지명 복사+오토라벨링 기본값 확인
> #4, 모델탭→학습탭 내 +버튼 모달 구조변경 #5, 채널전환/어노테이션추가 시 깜빡임 #6 —
> #6은 제목"로딩 느림"과 본문"깜빡임" 불일치 확인해 기획에 명시) → planner 위임(백그라운드,
> 디자인 톤 재검토와 파일 안 겹쳐 병행 가능). #5는 탭 기반 설계 원칙과 충돌 가능한 구조
> 변경이라 즉시 결정 대신 사용자 확인 항목으로 분류하도록 지시함. **결과 대기 중.**
>
