...
> **검증 진행 중**(골든패스 명시 요청함). **⑥~⑦ 미착수**, ⑤ 검증 통과 후 ⑥(i18n en전환)
> 진행 예정.
> 부수 발견 버그 3건 등록: **BUG-005**(P2, 라벨링 탭 폴더그룹핑이 `reload()` 비재귀+
> 이미지추가 평탄복사로 실제 도달 불가능한 죽은 코드, GitHub #1 VOC 응답 정정 필요 —
> `decisions-needed.md`에 처리방향 3안 등록, **사용자 결정 대기**, 7단계와 독립이라
> 병행 가능) / **BUG-006**(P2, model_tab 검증기는 허용외 import를 경고만 하고 통과시키나
> 로더는 무조건 실패 — 정책 불일치, 이번 라운드 회귀 아님) / **BUG-007**(P3, loss_chart
> 그리드·epoch선 색 중복 — 리더가 직접 1줄 수정해 해결, Closed).
>
