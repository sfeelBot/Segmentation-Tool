...
- [ ] **#6-B 어노테이션 개수 증가 시 로딩 지연 (최우선, 사용자 명시)** — `labeling_tab.py`
      ...
      **검증 대기** — 구현 에이전트가 이번 세션 인터프리터의 QtSvg DLL 로딩 문제로
      `python main.py` 단독 기동 확인을 못함(clean HEAD에서도 재현되는 기존 환경
      이슈로 확인됨, 이번 라운드 회귀 아님 — 이전 R4/R5·GitHub 라운드2 검증 로그에도
      동일 클래스 이슈 기록됨). 골든패스 육안 확인 + #6-A 회귀 확인 + 대규모 프로젝트
      체감 개선 확인 필요.
