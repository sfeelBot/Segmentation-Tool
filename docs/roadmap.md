...
- **2026-08-20 사용자 승인 완료** — 통합안대로 구현 착수 확정. 새 색상 팔레트 도입 없음
  ...
  4. [x] `loss_chart.py` matplotlib 배색을 앱 톤에 맞게 조정 (발견 2) — 구현+독립검증
        ...
        기록.
  5. [ ] 학습 탭 큐 서브스플리터 + 추론 탭 서브스플리터·이미지 트리 — 병렬 구현 중.
        **학습 탭 부분 구현 완료**: 커밋 `a32a8b5`(feat)+`b51318f`(docs), `_queue_splitter`
        세로 QSplitter 전환(라벨링 탭 hover색 패턴 재사용) + `_queue_list` 높이 상한 제거
        같은 커밋에서 함께 처리, `setSizes([260,10000])`로 기존 높이 재현. 추론 탭 부분은
        구현 에이전트 작업 중. **양쪽 다 끝나면 함께 검증** 예정(추론 탭 완료 대기).
