...
- [x] R3-4 — Undo 통합(원편집+블랍삭제+브러시지우기, Ctrl+Z+버튼). 구현 완료(2026-08-27,
      `ZoneCanvas._undo_stack` 통합 스택 + `_push_undo()`/`undo()`/`can_undo()`, 마스크가
      아니라 경량 스냅샷(원 튜플/블랍id 집합/지우기 스트로크 좌표)만 저장 — BUG-014 재발
      없음, 캡도 없음). 구현자 자체 QTest 스크립트(마우스 이벤트 실제 재현)로 원생성/
      블랍삭제/브러시지우기 각각의 undo + 3가지 혼합 LIFO 순서 + `set_blob_data` 시 스택
      리셋 확인 완료 — **실제 GUI 조작 검증은 미실시, 검증 에이전트 확인 필요**.
