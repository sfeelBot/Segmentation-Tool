...
- [x] **#6-B 어노테이션 개수 증가 시 로딩 지연 (최우선, 사용자 명시)** — `labeling_tab.py`
      ...
      **구현+독립검증 통과**(커밋 `41eaf13`) — 실제 `python main.py` 기동 성공(QtSvg
      ...
      (#5만 사용자 결정으로 보류).
      **2026-08-20 추가 요청 — 실사용 규모 재검증**: 사용자가 "500개 이상, 이미지
      만개 이상×이미지당 50개 이상"의 현실적 대규모 시나리오로 재확인 요청 → 구현
      에이전트에 위임(백그라운드), 두 축(단일 이미지 어노테이션 500+ 스케일링 유지
      여부 / 10,000+ 이미지 프로젝트 전체 브라우저·검색·전환 성능 + BUG-014 실제
      발현 여부) 확인 중.
