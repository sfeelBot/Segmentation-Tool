...
- [ ] **#6-B 어노테이션 개수 증가 시 로딩 지연 (최우선, 사용자 명시)** — `labeling_tab.py`
      ...
      재렌더링) 2곳의 O(n) 전체 재구축 패턴이 원인으로 특정됨. **구현 착수** — 성능개선
      R1~R6와 동일 패턴(벤치마크→원인확정→최소침습 수정→재벤치마크)으로 구현 에이전트에
      위임, 진행 중. #6-A(깜빡임, 커밋`6a823a5`)·BUG-011/012 수정분(커밋`24e93c9`/
      `5d551c3`)은 건드리지 말고 그 위에 성능 수정만 얹도록 지시함.
