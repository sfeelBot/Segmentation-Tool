...
- [ ] 라운드 1 — 제약값 조정 + 스타일 통일 (목업 불필요, 저리스크) — 구현 착수 가능
- [ ] 라운드 2 — 레이아웃 재구조화 + 추론 탭 이미지 목록 검색/정렬/트리형 폴더 신규 (디자인
      목업 필요, 중리스크) — 목업 착수 가능
- [ ] 라운드 3 — 정보 추가 (디자인 목업 필요, 중~고리스크 — `annotation_canvas.py`,
      `inference_engine.py` 최소 침습 패치 포함)
- 다음 단계: 라운드 1 구현 에이전트 위임 + 라운드 2 디자인 에이전트 목업 병행 착수.
