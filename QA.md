...
| BUG-011 | P1 | 브러시 계열 도구(브러시/버킷/지우개/영역지우개) 활성 상태에서 캔버스를 더블클릭해 브러시 크기 다이얼로그(GitHub #3(b), 커밋 `29248fa`)를 열면, 더블클릭의 첫 클릭(mousePress+Release)이 `mouseDoubleClickEvent()`보다 먼저 Qt에 전달되어 **정상적인 단일 브러시 다이얼로그가 그대로 캔버스에 그려짐** — 클릭 지점에 원치 않는 `[Mask]` 어노테이션이 하나 추가된다. 다이얼로그에서 OK를 누르든 Cancel을 누르든 이 stray 마스크는 그대로 남고(재현: `annotation_canvas.py` `mouseDoubleClickEvent()`가 브러시 페인트 자체를 막는 로직이 없음), Undo(1회)로만 제거 가능 — 즉 **이 기능을 쓸 때마다 매번 의도치 않은 라벨 데이터가 추가되고 사용자가 수동으로 되돌려야 함**. 실측: `python main.py` 실제 GUI로 브러시 도구 선택 후 캔버스 더블클릭 2회 반복, 매번 Annotation List에 `#N [Mask] object` 신규 항목 생성 확인(스크린샷 대조) | GitHub #3/#4/#6-A/#7 라운드2 검증(verifier) | Open |
| BUG-012 | P2 | 라벨링 탭에서 GitHub #7(OK 확인 팝업, 커밋 `29248fa`)의 "예" 경로(라벨 삭제 후 OK 처리)를 실행한 직후에는 이미지 브라우저 사이드바 아이콘이 정확히 체크마크(OK)로 갱신되지만, 다른 이미지로 전환했다가 다시 돌아오면 사이드바 아이콘이 "미라벨"(빈 원)로 되돌아간다 — 단, 디스크의 어노테이션 JSON(`annotations: [], ok: true`)과 툴바의 OK 액션 체크 상태는 둘 다 정확하게 "OK"를 유지하고 있어 **데이터 유실은 아니고 사이드바 아이콘만 stale**해지는 표시 버그. `annotation_canvas.py`의 `_do_save()`가 매번 새 daemon 스레드로 `store.save()`를 비동기 실행하면서 동시에 `annotation_saved` 시그널을 동기로 emit하고, `_on_toggle_ok()`(클리어→즉시 flush→`toggle_ok()`) 한 번의 클릭에서 `annotation_saved`가 최소 2회 emit되는 구조라 `image_browser.refresh_item()`이 서로 다른 시점의 상태를 반복 반영하다가 최종적으로 stale한 캐시가 남는 것으로 추정(로그 타임스탬프 상 500ms 디바운스 `_save_timer`가 지연 발화하며 다른 이미지 컨텍스트에서 `refresh_item`을 재호출하는 정황 확인, 정확한 트리거 라인은 미확정 — 후속 조사 필요). 재현: `python main.py`로 라벨 있는 이미지에서 OK 토글→"예"→체크마크 확인(스크린샷)→다른 이미지 클릭→원래 이미지로 복귀→사이드바 아이콘이 빈 원으로 회귀(스크린샷), 동시에 `projects/nok/annotations/*.json`은 `ok: true`로 정확 | GitHub #3/#4/#6-A/#7 라운드2 검증(verifier) | Open |
| BUG-013 | P3 | GitHub #3(b) 브러시 크기 다이얼로그(`annotation_canvas.py` `mouseDoubleClickEvent()`, `QInputDialog.getInt(self, "브러시 크기", "브러시 크기 (1~200)", ...)`)의 제목/라벨 문자열이 `app/core/i18n.py`의 `t()` 체계를 거치지 않고 한국어로 하드코딩돼 있음 — 언어 설정을 English로 바꿔도 이 다이얼로그만 한국어로 표시됨(같은 화면의 툴바 "Brush size:" 라벨은 `tool.brush_size` 키로 정상 영역화됨과 대비). "아이콘/이모지 → 미니멀 디자인 + i18n(en) 완비" 라운드에서 en 키 193개를 1:1 정비했던 것과 별개로 이번 라운드에서 신규 추가된 문자열이라 그 정비 범위 밖에 있었음 | GitHub #3/#4/#6-A/#7 라운드2 검증(verifier) | Open |
