...
| BUG-007 | P3 | `loss_chart.py` 배색 정규화(커밋 e76361a)에서 그리드(`_GRID`)와 epoch 경계 세로선(`_EPOCH`)이 동일한 `#374151`로 통일됨 — 기존엔 `_GRID=#3a3a3a`/`_EPOCH=#555555`로 서로 다른 명도라 epoch 경계선이 그리드보다 밝게 도드라져 보였지만, 통일 이후에는 alpha(0.6 vs 1.0)·선굵기(0.4 vs 0.6)만 미세하게 달라 실제 렌더링에서 epoch 경계 세로선이 일반 x축 그리드선과 거의 구분되지 않음(더미 데이터로 실제 렌더링해 확대 스크린샷으로 확인). 데이터 판독(학습/검증 손실 값, 두 선 색 구분)에는 영향 없고 이번 라운드 색상 매핑표에 "그리드와 통일"로 명시된 의도된 절충이라 기능 결함은 아니나, epoch 경계 표시라는 원래 목적은 사실상 무력화됨 — 필요 시 `_EPOCH`에 그리드보다 밝은 별도 톤(예: `_TEXT_MUTED` 계열) 부여 검토 | loss_chart.py 배색 조정 라운드 검증(verifier) | Open |
